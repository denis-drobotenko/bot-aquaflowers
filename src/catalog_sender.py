"""
Отправка каталога товаров через WhatsApp
"""

import logging
import httpx
from src.config import WHATSAPP_TOKEN, WHATSAPP_PHONE_ID
from src.catalog_reader import get_catalog_products, filter_available_products
from src.utils.logging_utils import ContextLogger

logger = ContextLogger("catalog_sender")
...
async def handle_send_catalog(to_number: str, sender_id: str = None, session_id: str = None):
    ...
    logger.info(f"[CATALOG_SEND] Отправка каталога пользователю {to_number}")
    
    try:
        catalog_products = get_catalog_products()
        available_products = filter_available_products(catalog_products)

        if not available_products:
            logger.warning("[CATALOG_SEND] Нет доступных товаров")
            from src.utils.whatsapp_client import WhatsAppClient
            whatsapp_client = WhatsAppClient()
            await whatsapp_client.send_text_message(to_number, "Извините, сейчас нет букетов в наличии. Попробуйте позже. 🌸")
            return False
        ...
        for product in available_products:
            ...
            logger.info(f"[CATALOG_SEND] Отправляем товар: {name} - {price}")
        ...
        from src.utils.whatsapp_client import WhatsAppClient
        whatsapp_client = WhatsAppClient()
        await whatsapp_client.send_text_message(to_number, "Произошла ошибка при отправке каталога. 🌸")
        return False
...
async def send_whatsapp_image_with_caption(to_number: str, image_url: str, caption: str, sender_id: str = None, session_id: str = None):
    ...
    try:
            ...
            if session_id and sender_id:
                try:
                    ...
                    if 'messages' in response_data and len(response_data['messages']) > 0:
                        ...
                        add_message(sender_id, session_id, "model", caption, message_id=message_id)
        ...
        whatsapp_client = WhatsAppClient()
        try:
            await whatsapp_client.send_text_message(to_number, caption)
        except:
            pass
        return False 