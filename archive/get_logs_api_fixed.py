...
def get_logs_via_api(start_time=None, end_time=None):
    """Получает логи за указанный диапазон времени через Google Cloud Logging API"""
    try:
        from google.cloud import logging
        print("Подключаюсь к Google Cloud Logging API...")
        client = logging.Client()
        # Если не переданы параметры — последние 24 часа
        if not end_time:
