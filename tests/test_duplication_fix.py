#!/usr/bin/env python3
"""
Тест исправления дублирования сообщений после подтверждения заказа
"""
...
@pytest.mark.asyncio
async def test_duplication_fix():
    """Тестирует исправление дублирования сообщений после подтверждения заказа"""
    ...
    try:
        ...
        print(f"3. Результат выполнения команды: {result}")
        ...
        # Тест 3: Проверяем, что новая сессия не генерирует автоматический ответ
        print("4. Проверяем, что новая сессия не генерирует автоматический ответ")
        
        # Проверяем функцию is_session_created_after_order
        is_after_order = session_manager.is_session_created_after_order(new_session_id)
        print(f"   is_session_created_after_order: {is_after_order}")
...
if __name__ == "__main__":
    asyncio.run(test_duplication_fix()) 